...
// Cache job categories to avoid repetitive DB calls and parsing overhead.
// This reduces query time and CPU usage for keyword parsing on every check.
const getCachedJobCategories = unstable_cache(
  async () => {
    const categories = await prisma.jobCategory.findMany()
    return categories.map((cat) => ({
      ...cat,
      // Pre-parse keywords to avoid repeated splitting in the hot loop
      parsedKeywords: cat.keywords.toLowerCase().split(',').map((k) => k.trim()),
    }))
  },
  ['job-categories-list'],
  { tags: ['job-categories'], revalidate: 3600 }
)
...
async function getMarketScore(jobTitle: string) {
  ...
  const categories = await getCachedJobCategories()
  ...
  for (const cat of categories) {
    if (cat.parsedKeywords.some((k) => lowerTitle.includes(k))) {
      matchedCategory = cat
      break
    }
  }
